  ...
  return (
    <div className="flex min-h-full flex-col overflow-x-hidden bg-cream">
      <header className="sticky top-0 z-40 border-b border-border bg-card/95 backdrop-blur-md">
        ...
        {/* Mobile: wrapping chips — no horizontal scroll */}
        <nav className="mx-auto flex max-w-6xl flex-wrap gap-1.5 px-3 pb-3 lg:hidden">
          {dashboardNav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="inline-flex rounded-lg bg-muted/60 px-2.5 py-1.5 text-xs font-medium text-foreground/80 hover:bg-muted hover:text-primary"
            >
              {item.short}
            </Link>
          ))}
          ...
          <ul className="space-y-1 text-sm">
            {dashboardNav.map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className="block rounded-lg px-3 py-2 text-foreground/80 hover:bg-muted hover:text-primary"
                >
                  {item.label}
                </Link>
              </li>
