...
export { RENTAL_COUNT } from "@/lib/rental-catalog";

/** Card grid for rentals — used by Rentals tab and embedded in All. */
export function RentalListingGrid({
  items,
  emptyLabel = "No rentals match your filters.",
  onClear,
...
export function MarketplaceRentals({
  ...
  const categories = useMemo(
    () => Array.from(new Set(RENTAL_ITEMS.map((i) => i.category))).sort(),
    []
  );

  const filtered = useMemo(
    () => filterRentalItems(RENTAL_ITEMS, { search, category }),
    [search, category]
  );
